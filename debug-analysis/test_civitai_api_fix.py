...
def test_civitai_api_detection():
    ...
    print("🔧 TESTING CIVITAI API DETECTION FIX")
    print("=" * 40)

    # Test Case 1: User's problematic file (should NOT match CivitAI API)
    print("1. Testing user's file (should NOT be CivitAI API):")
...
Steps: 30, Sampler: dpmpp_2m_sde_gpu, CFG scale: 4.0, Seed: 1423235703, Size: 768x1216, Model: SDXL_Pony\\Myrij_-_α.safetensors, Model hash: 174381e92161"""
    ...
    print(f"   Rule 1 (A1111 parameters): {'✅ PASS' if rule1_matches else '❌ FAIL'}")

    # Test Rule 2 (CivitAI API indicators) - OLD patterns
    old_patterns = ["illustrious", "pony", "ponydiffusion", "autismmix"]
    old_rule2_matches = any(pattern.lower() in user_content.lower() for pattern in old_patterns)
    print(f"   Rule 2 OLD (broad patterns): {'❌ WOULD MATCH' if old_rule2_matches else '✅ WOULD NOT MATCH'}")
    ...
    new_rule2_matches = any(re.search(pattern, user_content, re.IGNORECASE) for pattern in new_patterns)
    print(f"   Rule 2 NEW (specific API indicators): {'❌ MATCH' if new_rule2_matches else '✅ NO MATCH'}")

    overall_match = rule1_matches and new_rule2_matches
    print(f"   Overall CivitAI API detection: {'❌ WOULD MATCH' if overall_match else '✅ WOULD NOT MATCH'}")

    if not overall_match:
        print("   ✅ SUCCESS: User's file will NOT be detected as CivitAI API!")
    else:
        print("   ❌ FAIL: User's file would still be detected as CivitAI API")
...
Steps: 25, Sampler: euler, CFG scale: 7.0, Seed: 123456, Size: 512x512, Model: 287520.safetensors, Model hash: abc123def456, Version: 1.2.3, API Key: sk_1234567890abcdef, Request ID: req_abc123"""
    ...
    rule2_api = any(re.search(pattern, api_content, re.IGNORECASE) for pattern in new_patterns)
    api_match = rule1_api and rule2_api

    print(f"   Rule 1 (A1111 parameters): {'✅ PASS' if rule1_api else '❌ FAIL'}")
    print(f"   Rule 2 (API indicators): {'✅ PASS' if rule2_api else '❌ FAIL'}")
    print(f"   Overall CivitAI API detection: {'✅ MATCH' if api_match else '❌ NO MATCH'}")

    if api_match:
        print("   ✅ SUCCESS: Real API file correctly detected!")
    else:
        print("   ❌ FAIL: Real API file not detected")
...
Steps: 20, Sampler: dpm_2, CFG scale: 6.5, Seed: 987654, Size: 1024x1024, Model: 445566.safetensors, Model hash: xyz789"""
    ...
    rule2_num = any(re.search(pattern, numbered_content, re.IGNORECASE) for pattern in new_patterns)
    num_match = rule1_num and rule2_num

    print(f"   Rule 1 (A1111 parameters): {'✅ PASS' if rule1_num else '❌ FAIL'}")
    print(f"   Rule 2 (numbered model): {'✅ PASS' if rule2_num else '❌ FAIL'}")
    print(f"   Overall CivitAI API detection: {'✅ MATCH' if num_match else '❌ NO MATCH'}")
...
Steps: 28, Sampler: euler_a, CFG scale: 7.5, Seed: 555666, Size: 768x768, Model: anythingv5.safetensors, Model hash: def456ghi"""
    ...
    rule2_reg = any(re.search(pattern, regular_content, re.IGNORECASE) for pattern in new_patterns)
    reg_match = rule1_reg and rule2_reg

    print(f"   Rule 1 (A1111 parameters): {'✅ PASS' if rule1_reg else '❌ FAIL'}")
    print(f"   Rule 2 (API indicators): {'✅ NO MATCH' if not rule2_reg else '❌ MATCH'}")
    print(f"   Overall CivitAI API detection: {'✅ NO MATCH' if not reg_match else '❌ MATCH'}")
    ...
    if passed_tests == total_tests:
        ...
        print("✅ Regular A1111 files won't be misidentified as CivitAI API")
        print("✅ Real API files will still be correctly detected")
    else:
