...
def test_swarmui_adobe_rejection():
    ...
    print("🔧 TESTING SWARMUI ADOBE REJECTION")
    print("=" * 38)
    
    # Test Case 1: Adobe Photoshop file (should be rejected)
    print("1. Testing Adobe Photoshop file rejection:")
    adobe_info = {
        "software_tag": "Adobe Photoshop 26.5 (20250212.m.2973 455b503)  (Macintosh)"
    }
    
    swarm_parser = SwarmUI(info=adobe_info, raw='{"software_tag": "Adobe Photoshop 26.5 (20250212.m.2973 455b503)  (Macintosh)"}')
    swarm_parser.parse()
    
    print(f"   Status: {swarm_parser.status}")
    print(f"   Error: {swarm_parser.error}")
    print(f"   Tool: {swarm_parser.tool}")
    ...
    swarmui_info = {
        "software_tag": "StableSwarmUI v0.6.2"
    }
    ...
    print(f"   Status: {swarm_parser2.status}")
    print(f"   Prompt: {swarm_parser2.positive}")
    print(f"   Tool: {swarm_parser2.tool}")
    
    if swarm_parser2.status == swarm_parser2.Status.READ_SUCCESS:
        print("   ✅ CORRECTLY ACCEPTED SwarmUI file!")
    else:
        print("   ❌ FAILED to accept legitimate SwarmUI file")
    
    # Test Case 3: Other Adobe software (should be rejected)
    print("\n3. Testing other Adobe software rejection:")
    other_adobe_info = {
        "software_tag": "Adobe Lightroom 13.1"
    }
    
    swarm_parser3 = SwarmUI(info=other_adobe_info, raw='{"software_tag": "Adobe Lightroom 13.1"}')
    swarm_parser3.parse()
    
    print(f"   Status: {swarm_parser3.status}")
    print(f"   Error: {swarm_parser3.error}")
    
    if swarm_parser3.status == swarm_parser3.Status.FORMAT_DETECTION_ERROR:
        print("   ✅ CORRECTLY REJECTED other Adobe software!")
    else:
        print("   ❌ FAILED to reject other Adobe software")
    
    # Test Case 4: Other non-AI software (should be rejected)
    print("\n4. Testing other non-AI software rejection:")
    gimp_info = {
        "software_tag": "GIMP 2.10.34"
    }
    
    swarm_parser4 = SwarmUI(info=gimp_info, raw='{"software_tag": "GIMP 2.10.34"}')
    swarm_parser4.parse()
    
    print(f"   Status: {swarm_parser4.status}")
    print(f"   Error: {swarm_parser4.error}")
    
    if swarm_parser4.status == swarm_parser4.Status.FORMAT_DETECTION_ERROR:
        print("   ✅ CORRECTLY REJECTED GIMP software!")
    else:
        print("   ❌ FAILED to reject GIMP software")
    ...
    print(f"   Status: {swarm_parser5.status}")
    print(f"   Prompt: {swarm_parser5.positive}")
    
    if swarm_parser5.status == swarm_parser5.Status.READ_SUCCESS:
        print("   ✅ CORRECTLY PROCESSED file without software tag!")
    else:
        print("   ❌ FAILED to process file without software tag")
    
    print("\n🎯 SUMMARY:")
    print("-" * 12)
    print("✅ SwarmUI parser should now correctly reject Adobe Photoshop files")
    print("✅ The user's issue should be resolved")
    print("✅ Adobe files will no longer be misidentified as StableSwarmUI")
