...
def test_a1111_jpeg_debug():
    ...
    print(  # noqa: T201"🔍 A1111 JPEG DEBUG TEST")
    print(  # noqa: T201"=" * 25)
    ...
    try:
        ...
        if user_comment:
            ...
            print(  # noqa: T201f"   A1111 patterns found: {found_patterns}")
        else:
        ...
        if pil_params:
            print(  # noqa: T201f"✅ PIL parameters found: {len(pil_params)} characters")
            print(  # noqa: T201f"   Preview: {pil_params[:100]}...")
        else:
    ...
    try:
        ...
        print(  # noqa: T201"Testing A1111 detection rules...")

        # Load A1111 parser definition
        import json

        a1111_parser_path = os.path.join(
            os.path.dirname(__file__), "parser_definitions", "a1111_webui.json"
        )
        with open(a1111_parser_path) as f:
            a1111_def = json.load(f)

        print(  # noqa: T201f"A1111 parser priority: {a1111_def.get('priority')}")
        print(  # noqa: T201f"Detection rules count: {len(a1111_def.get('detection_rules', []))}")
        ...
        for i, rule in enumerate(a1111_def.get("detection_rules", [])):
            ...
            try:
                ...
                print(  # noqa: T201f"   Result: {'✅ PASS' if result else '❌ FAIL'}")

                if not result and rule.get("operator") in ["AND", "OR"]:
                    # Debug composite rules
                    print(  # noqa: T201"   Debugging composite rule...")
                    for j, sub_rule in enumerate(rule.get("rules", [])):
                        print(  # noqa: T201
                            f"     Sub-rule {j + 1}: {sub_rule.get('comment', 'No comment')}"
                        )
                        try:
                            sub_result = evaluator.evaluate_rule(sub_rule, context)
                            print(  # noqa: T201
                                f"     Result: {'✅ PASS' if sub_result else '❌ FAIL'}"
                            )
                        except Exception as sub_e:
                            print(  # noqa: T201f"     Error: {sub_e}")
                elif (
                    not result
                    and rule.get("source_type") == "a1111_parameter_string_content"
                ):
                    # Debug the a1111_parameter_string_content source
                    print(  # noqa: T201"   Debugging a1111_parameter_string_content...")
                    source_data, source_found = evaluator._get_source_data_and_status(
                        rule, context
                    )
                    print(  # noqa: T201f"   Source found: {source_found}")
                    if source_data:
                        print(  # noqa: T201f"   Source data length: {len(source_data)}")
                        print(  # noqa: T201f"   Source preview: {source_data[:100]}...")

                        # Test the regex patterns manually
                        if rule.get("operator") == "regex_match_all":
                            patterns = rule.get("regex_patterns", [])
                            print(  # noqa: T201f"   Testing patterns: {patterns}")
                            for pattern in patterns:
                                found = pattern in source_data
                                print(  # noqa: T201f"     '{pattern}': {'✅' if found else '❌'}")

            except Exception as e:
                print(  # noqa: T201f"   Error: {e}")

    except Exception as e:
        print(  # noqa: T201f"❌ Rule evaluation error: {e}")
    ...
    try:
        ...
        if result:
            print(  # noqa: T201f"✅ MetadataEngine result: {type(result)}")
            if isinstance(result, dict):
                tool = result.get("tool", "Unknown")
                print(  # noqa: T201f"   Tool detected: {tool}")

                if tool == "Automatic1111 WebUI":
                    print(  # noqa: T201"   ✅ Correctly identified as A1111")
                    ...
                    print(  # noqa: T201f"   Prompt: {prompt[:50]}..." if prompt else "   No prompt")
                    print(  # noqa: T201f"   Steps: {steps}")
                    print(  # noqa: T201f"   Seed: {seed}")
                else:
                    print(  # noqa: T201f"   ❌ Incorrectly identified as: {tool}")
            else:
                print(  # noqa: T201f"   ❌ Unexpected result type: {type(result)}")
        else:
            print(  # noqa: T201"❌ MetadataEngine returned None")
    ...
    print(  # noqa: T201"\n🎯 EXPECTED BEHAVIOR:")
    print(  # noqa: T201"   • Context should extract A1111 parameters from UserComment")
    print(  # noqa: T201"   • A1111 parser should detect the parameter patterns")
    print(  # noqa: T201"   • MetadataEngine should identify as 'Automatic1111 WebUI'")
    print(  # noqa: T201"   • All A1111 fields should be extracted correctly")
