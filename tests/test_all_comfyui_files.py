#!/usr/bin/env python3
"""Test ALL ComfyUI files to see the real extraction success rate."""
...
def test_all_comfyui_files():
    """Test all ComfyUI files in the directory."""
    
    test_dir = Path("/Users/duskfall/Desktop/Comfy_UI_DATA")
    ...
    print(f"🔍 Testing {len(png_files)} PNG files...")
    print("="*100)
    ...
    for i, png_file in enumerate(png_files[:20]):  # Test first 20 files
    ...
    if results['failed_extraction'] > 0:
        print(f"\n🚨 ISSUE CONFIRMED: {results['failed_extraction']} ComfyUI files are not extracting prompts!")
    else:
        print(f"\n🎉 SUCCESS: All ComfyUI files are extracting prompts correctly!")

if __name__ == "__main__":
    test_all_comfyui_files()