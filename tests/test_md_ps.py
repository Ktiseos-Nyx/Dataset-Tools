...
import json
import unittest
from unittest.mock import MagicMock, mock_open, patch # Added MagicMock
...
class TestParseMetadataIntegration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.comfy_example_prompt_str = json.dumps({
            "3": {
                "class_type": "KSampler",
                "inputs": {
                    "seed": 987654321, "steps": 25, "cfg": 8.0, 
                    "sampler_name": "euler", "scheduler": "normal", "denoise": 1.0,
                    "model": ["4", 0], "positive": ["6", 0], "negative": ["7", 0],
                    "latent_image": ["5", 0],
                },
            },
            "4": {"class_type": "CheckpointLoaderSimple", "inputs": {"ckpt_name": "comfy_sdxl_model.safetensors"}},
            "5": {"class_type": "EmptyLatentImage", "inputs": {"width": 768, "height": 1024, "batch_size": 1}},
            "6": {"class_type": "CLIPTextEncode", "inputs": {"text": "positive prompt for ComfyUI test", "clip": ["4", 1]}},
            "7": {"class_type": "CLIPTextEncode", "inputs": {"text": "negative prompt for ComfyUI test", "clip": ["4", 1]}},
        })
    ...
    def test_parse_metadata_a1111_success(self, mock_image_data_reader: MagicMock) -> None:
        ...
        mock_reader_instance.parameter = {
            "model": "test_model_a1111.safetensors", "model_hash": "abcdef1234",
            "sampler_name": "DPM++ 2M Karras", "seed": "1234567890",
            "cfg_scale": "7.5", "steps": "30", "size": "512x768", "clip_skip": "2",
        }
        ...
        self.assertEqual(result[UpField.PROMPT.value].get("Positive"), "positive prompt test from a1111")
        self.assertEqual(result[UpField.PROMPT.value].get("Negative"), "negative prompt test from a1111")
    ...
    def test_process_pyexiv2_data_full(self, mock_nfo_logger: MagicMock) -> None: # Renamed and typed mock
        ...
        pyexiv2_input = {
            "EXIF": {
                "Exif.Image.Make": "Canon", "Exif.Image.Model": "EOS R5",
                "Exif.Photo.DateTimeOriginal": "2023:01:01 10:00:00",
                "Exif.Photo.UserComment": "Standard User Comment test",
                "Exif.Photo.FNumber": 2.8, "Exif.Photo.ISOSpeedRatings": 100,
            },
            "XMP": {
        ...
        expected_output = {
            DownField.EXIF.value: {
                "Camera Make": "Canon", "Camera Model": "EOS R5",
        ...
        if DownField.EXIF.value in expected_output:
            ...
            self.assertDictEqual(actual_output[DownField.EXIF.value],expected_output[DownField.EXIF.value],"EXIF data mismatch")
        else:
        ...
        if UpField.TAGS.value in expected_output:
            ...
            self.assertDictEqual(actual_output[UpField.TAGS.value],expected_output[UpField.TAGS.value],"TAGS data mismatch")
        else:
        ...
        if DownField.EXIF.value in actual_output_ai_parsed:
            self.assertNotIn("Usercomment (std.)",actual_output_ai_parsed[DownField.EXIF.value],"UserComment should be skipped")
