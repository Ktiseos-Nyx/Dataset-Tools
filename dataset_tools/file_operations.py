...
from .logger import get_logger, debug_monitor
...
class FileExtensionCategories:
    ...
    def __init__(self):
        ...
        self._image_extensions: Optional[Set[str]] = None
        self._text_extensions: Optional[Set[str]] = None
        self._model_extensions: Optional[Set[str]] = None
        self._ignore_list: Optional[List[str]] = None
    ...
    def _load_image_extensions(self) -> Set[str]:
        ...
        try:
            ...
            return {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.tiff', '.tif'}  # Fallback
    ...
    def categorize_file(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            if suffix in self.image_extensions:
                ...
                return 'text'
            elif suffix in self.model_extensions:
                return 'model'
            else:
                return None
...
class DirectoryScanner:
    ...
    def __init__(self, categorizer: Optional[FileExtensionCategories] = None):
        """
        ...
        try:
            ...
            if items is None:
                return FileScanResult(
                    images=[], texts=[], models=[], folder_path=folder_path,
                    total_files=0, scan_success=False, 
            ...
            self.logger.info(
                f"Scan completed for {folder_path}: "
                f"{len(images)} images, {len(texts)} texts, {len(models)} models"
            )
            ...
            self.logger.error(f"Unexpected error scanning directory '{folder_path}': {e}", exc_info=True)
            return FileScanResult(
                images=[], texts=[], models=[], folder_path=folder_path,
                total_files=0, scan_success=False,
    ...
    def _get_directory_items(self, folder_path: str) -> Optional[List[str]]:
        """
