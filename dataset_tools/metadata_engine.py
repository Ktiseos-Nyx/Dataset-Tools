...
# PASTE THIS ENTIRE BLOCK INTO metadata_engine.py, REPLACING THE OLD FUNCTION
    ...
    def _execute_field_extraction_method(
        ...
        def get_source_data(source_definition, mtd_input_data, ctx_data, fields_cache):
            if not source_definition: return mtd_input_data
            src_type = source_definition.get("type")
            src_key = source_definition.get("key")
            if src_type == "pil_info_key": return ctx_data.get("pil_info", {}).get(src_key)
            if src_type == "png_chunk": return ctx_data.get("png_chunks", {}).get(src_key)
            if src_type == "exif_user_comment": return ctx_data.get("raw_user_comment_str")
            if src_type == "xmp_string": return ctx_data.get("xmp_string")
            if src_type == "file_content_raw_text": return ctx_data.get("raw_file_content_text")
            if src_type == "file_content_json_object": return ctx_data.get("parsed_root_json_object")
            if src_type == "direct_context_key": return ctx_data.get(src_key)
                    ...
                    if kv_block_str:
                        try:
                            ...
                            if first_line_of_kv:
                                ...
                                if kv_start_index_in_original != -1: end_index = min(end_index, kv_start_index_in_original)
                        except (ValueError, AttributeError): pass
                    value = data_for_method[:end_index].strip()

            elif method_name == "a1111_extract_prompt_negative":
                if isinstance(data_for_method, str):
                    neg_match = re.search(r"\nNegative prompt:(.*?)(?=(\n(?:Steps:|Sampler:|CFG scale:|Seed:|Size:|Model hash:|Model:|Version:|$)))", data_for_method, re.IGNORECASE | re.DOTALL)
        ...
        if value is not None and value_type:
            try:
                if value_type == "integer": value = int(float(str(value)))
               ...
               (not detection_rules and target_types == ["*"]):
                ...
                for rule_idx, rule in enumerate(detection_rules): # Outer loop for rules in parser_def
                    ...
                    if isinstance(rule, dict) and "condition" in rule and isinstance(rule.get("rules"), list): # Added check for list
                        ...
                        # For a complex rule (AND/OR block), after evaluating it, we should break
                        # from the loop processing THIS rule, as the block itself is one "rule entry".
                        # The 'all_rules_pass' will determine if we continue to the next parser_def.
                        # However, if all_rules_pass became False, we need to break the outer loop.
                        # The break here should be from the for rule_idx, rule loop if all_rules_pass is False
                        # Or if the complex rule itself determined the outcome for this parser_def.
                        # Your original 'break' was correct here: if complex rule fails, this parser_def fails.
                        # If complex rule passes, and there are MORE rules after it for this parser_def, we continue.
                        # But usually, a complex rule IS the entire set of rules, or one of a few.
                        # Let's assume the complex rule is one rule among potentially others.
                        # The crucial part is setting all_rules_pass correctly.
                        if not all_rules_pass: # If this complex rule made it fail
                            break # Break from the 'for rule_idx, rule in enumerate(detection_rules):' loop

                        # If the complex rule passed (passed_block is True, all_rules_pass still True),
                        # we should continue to the next rule in detection_rules (if any).
                        # So, no 'break' here if passed_block is True.
                        # The existing 'if not passed_block: all_rules_pass = False; break' logic
                        # was slightly off. It should be:
                        # if not passed_block:
                        #     all_rules_pass = False
                        # // and then the outer loop's break condition will handle it.
                        # The 'break' here means this complex rule is the *only* one for this parser_def or the last one considered.
                        # Let's stick to your original structure's break meaning: if complex rule makes it fail, stop checking this parser_def.
