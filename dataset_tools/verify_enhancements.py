...
def verify_enhancements():
    ...
    try:
        ...
        if t5_parser_path.exists():
            ...
            for rule in detection_rules:
                ...
                if class_types:
                    ...
                    advanced_nodes = [node for node in class_types if any(keyword in node for keyword in ["Flux", "PixArt", "Aura", "Hunyuan"])]
                    if advanced_nodes:
        ...
        if flux_parser_path.exists():
            ...
            enhanced_fields = [f for f in fields if any(keyword in f.get("target_key", "") for keyword in ["t5_model", "clip_model", "guidance"])]
