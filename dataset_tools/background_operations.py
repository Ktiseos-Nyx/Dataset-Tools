...
from .file_operations import FileOperations, FileScanResult
...
class FileLoaderTask(BackgroundTask):
    ...
    def __init__(self, folder_path: str, file_to_select: Optional[str] = None, parent=None):
        """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.emit_error(f"Unexpected error during file loading: {str(e)}")
...
class FileLoaderResult:
    ...
    def __init__(
        ...
        models: list[str],
        folder_path: str,
        file_to_select: Optional[str] = None,
        total_files: int = 0,
    ...
    def get_category_for_file(self, filename: str) -> Optional[str]:
        ...
        if filename in self.images:
            return 'image'
        elif filename in self.texts:
            return 'text'
        elif filename in self.models:
            return 'model'
        return None
        ...
        try:
            ...
            for i, file_path in enumerate(self.file_paths):
                ...
                try:
                    ...
                    self.emit_progress(progress, f"Processing {i+1}/{total_files}: {Path(file_path).name}")
            ...
            self.emit_error(f"Unexpected error during metadata loading: {str(e)}")
...
class TaskManager(QtCore.QObject):
    ...
    def start_task(self, task: BackgroundTask, task_id: Optional[str] = None) -> str:
        """
...
def load_files_in_background(
    ...
    completion_callback: Optional[Callable[[FileLoaderResult], None]] = None,
