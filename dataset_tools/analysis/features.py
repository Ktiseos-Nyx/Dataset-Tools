...
def check_for_flux(parser_instance: ImageReaderInstance | None) -> bool:
    ...
    if "flux" in params.get("model", "").lower(): return True
    resources = params.get("civitai_resources") or params.get("civitai_resources_data")
    if isinstance(resources, list):
        for res in resources:
            if isinstance(res, dict) and "flux" in res.get("modelName", "").lower(): return True
    raw_data = getattr(parser_instance, "raw", "")
    if "ComfyUI" in getattr(parser_instance, "tool", "") and isinstance(raw_data, str):
        if '"class_type": "FluxGuidance"' in raw_data or '"unet_name": "flux' in raw_data: return True
    return False
...
def check_for_ponyx(parser_instance: ImageReaderInstance | None) -> bool:
    ...
    if "ponyxscores" in prompt_lower or "ponyxv6_scores" in prompt_lower: return True
    if re.search(r'score_\d+(_up)?', prompt_lower): return True
...
def check_for_illustriousxl(parser_instance: ImageReaderInstance | None) -> bool:
    ...
    if not _is_parser_report_valid(parser_instance): return False
    params = getattr(parser_instance, "parameter", {})
    if "illustrious" in params.get("model", "").lower(): return True
    resources = params.get("civitai_resources") or params.get("civitai_resources_data")
    if isinstance(resources, list):
        for res in resources:
            if isinstance(res, dict) and res.get("type") == "checkpoint":
                if "illustrious" in res.get("modelName", "").lower(): return True
    return False
...
def check_for_lora(parser_instance: ImageReaderInstance | None) -> bool:
    ...
    if not _is_parser_report_valid(parser_instance): return False
    params = getattr(parser_instance, "parameter", {})
    if params.get("Lora hashes") or params.get("Lora hashes data"): return True
    prompt = getattr(parser_instance, "positive", "")
...
def check_for_sd15(parser_instance: ImageReaderInstance | None) -> bool:
    ...
    try:
        width = int(params.get("Width", 0)); height = int(params.get("Height", 0))
        if (width * height) >= MIN_VIABLE_SDXL_AREA: score += 2
        else: score -= 1
...
def analyze_features(parser_instance: ImageReaderInstance | None) -> FeatureAnalysisResult | None:
    """
