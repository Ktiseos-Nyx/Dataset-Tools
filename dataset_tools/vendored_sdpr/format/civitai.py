...
class CivitaiComfyUIFormat(BaseFormat):
           ...
           not (data_to_process.startswith('{') and data_to_process.endswith('}')):
            ...
            try:
                byte_list = [((ord(c) >> 8) & 0xFF) for c in data_to_process] + \
                            [ (ord(c)       & 0xFF) for c in data_to_process] # Simplified assumption, may need proper pairs
    ...
    def parse(self): # Overrides BaseFormat.parse directly
        if self.status == self.Status.READ_SUCCESS: return self.status # Already parsed

        self._logger.info(f"Attempting to parse as {self.tool}.")
        if not self._raw: 
            self._logger.warn("Raw UserComment data is empty."); self.status = self.Status.FORMAT_ERROR; self._error = "Raw UserComment empty."; return self.status 

        cleaned_workflow_json_str = self._decode_user_comment_for_civitai(self._raw)

        if not cleaned_workflow_json_str:
            self._logger.warn("Failed to decode UserComment or not Civitai JSON."); self.status = self.Status.FORMAT_ERROR; self._error = "UserComment decode/validation failed."; return self.status
        ...
        try:
            ...
            self._logger.error(f"Decoded UserComment is not valid JSON: {e}"); self.status = self.Status.FORMAT_ERROR; self._error = f"Invalid JSON: {e}"; return self.status

        extra_metadata_str = self.workflow_data.get("extraMetadata")
        if not (extra_metadata_str and isinstance(extra_metadata_str, str)):
            self._logger.warn("'extraMetadata' not found or not a string."); self.status = self.Status.FORMAT_ERROR; self._error = "'extraMetadata' missing/invalid."; return self.status
        ...
        try:
            ...
            if "steps" in self._parameter and extra_meta_dict.get("steps") is not None: self._parameter["steps"] = str(extra_meta_dict["steps"])
            
            cfg_val = extra_meta_dict.get("CFG scale", extra_meta_dict.get("cfgScale"))
            if "cfg_scale" in self._parameter and cfg_val is not None: self._parameter["cfg_scale"] = str(cfg_val)
            
            sampler_val = extra_meta_dict.get("sampler", extra_meta_dict.get("sampler_name"))
            if "sampler_name" in self._parameter and sampler_val is not None: self._parameter["sampler_name"] = str(sampler_val)
            
            if "seed" in self._parameter and extra_meta_dict.get("seed") is not None: self._parameter["seed"] = str(extra_meta_dict["seed"])
            
            if extra_meta_dict.get("width") is not None: self._width = str(extra_meta_dict.get("width"))
            if extra_meta_dict.get("height") is not None: self._height = str(extra_meta_dict.get("height"))
            ...
            for k, v in extra_meta_dict.items():
                if k not in ["prompt", "negativePrompt", "steps", "CFG scale", "cfgScale", "sampler", "sampler_name", "seed", "width", "height"]:
            ...
            self._logger.error(f"Failed to parse JSON from 'extraMetadata': {e_extra}"); self.status = self.Status.FORMAT_ERROR; self._error = f"Invalid 'extraMetadata' JSON: {e_extra}"
        except Exception as e_final:
            self._logger.error(f"Unexpected error processing Civitai 'extraMetadata': {e_final}", exc_info=True); self.status = self.Status.FORMAT_ERROR; self._error = f"Processing error: {e_final}"
