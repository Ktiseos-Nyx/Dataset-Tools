...
class NovelAI(BaseFormat):
    ...
    def __init__(
        self, info: dict = None, raw: str = "", extractor: LSBExtractor = None, # Changed from width, height to extractor
        width: int = 0, height: int = 0 # Keep width/height from ImageDataReader for legacy if info is used
    ):
    ...
    def _parse_nai_legacy_png(self) -> bool:
        try:
            ...
            if not isinstance(data_json, dict):
                self._error = "Legacy NovelAI 'Comment' field is not a valid JSON dictionary."; return False

            self._negative = str(data_json.get("uc", "")).strip() # Negative prompt from "uc"

            # Populate self._parameter
            param_map = {"sampler": "sampler_name", "seed": "seed", "strength": "denoising_strength", # NAI "strength" is like denoise
            ...
            self._error = f"Invalid JSON in NovelAI legacy 'Comment': {e}"; return False
        except Exception as e:
            self._error = f"Error parsing NovelAI legacy PNG: {e}"; self._logger.error(f"NovelAI legacy parsing error: {e}", exc_info=True); return False
    ...
    def _parse_nai_stealth_png(self) -> bool:
        try:
            ...
            if data_length_bytes is None:
                self._error = "Could not read data length from LSB stream."; return False
            ...
            if compressed_data is None or len(compressed_data) != data_length_bytes:
                self._error = f"Could not read {data_length_bytes} bytes of compressed data from LSB stream."; return False

            json_string = gzip.decompress(compressed_data).decode("utf-8")
            data_json = json.loads(json_string)
            if not isinstance(data_json, dict):
                self._error = "Decompressed LSB data is not a valid JSON dictionary."; return False
            ...
            param_map = {"sampler": "sampler_name", "seed": "seed", "strength": "denoising_strength",
                         "noise": "noise_offset", "scale": "cfg_scale", "steps": "steps",
                         "width": "width", "height": "height"} # NAI JSON often has width/height
            ...
            if data_for_params.get("width") is not None: self._width = str(data_for_params.get("width"))
            if data_for_params.get("height") is not None: self._height = str(data_for_params.get("height"))
            if "size" in self._parameter and self._width != "0" and self._height != "0":
                 self._parameter["size"] = f"{self._width}x{self._height}"
            ...
            self._error = f"Invalid GZip data in NovelAI stealth PNG: {e_gzip}"; return False
        except json.JSONDecodeError as e_json:
            self._error = f"Invalid JSON in NovelAI stealth PNG: {e_json}"; return False
        except Exception as e:
            self._error = f"Error parsing NovelAI stealth PNG: {e}"; self._logger.error(f"NovelAI stealth parsing error: {e}", exc_info=True); return False