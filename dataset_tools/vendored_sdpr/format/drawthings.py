...
class DrawThings(BaseFormat):
    ...
    def _process(self): # Called by BaseFormat.parse()
        ...
        try:
            ...
            temp_raw_parts = []
            if self._positive: temp_raw_parts.append(f"Positive: {self._positive}")
            if self._negative: temp_raw_parts.append(f"Negative: {self._negative}")
            ...
            try:
                ...
                self._logger.warn(f"Could not parse DrawThings Size '{size_str}'. Using defaults: {self._width}x{self._height}")
