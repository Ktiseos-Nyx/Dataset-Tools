...
class InvokeAI(BaseFormat):
    ...
    DREAM_MAPPING = { # For parsing the "-X value" style "Dream" string
        "Steps": "s", "Seed": "S", "CFG scale": "C", "Sampler": "A",
    ...
    def _parse_invoke_metadata_format(self) -> bool: # New name for original _invoke_metadata
        try:
            ...
            if not isinstance(data_json, dict):
                self._error = "invokeai_metadata is not a valid JSON dictionary."; return False
            ...
            if self._positive_sdxl or self._negative_sdxl: self._is_sdxl = True
            ...
            # Other parameters
            param_map = {"seed": "seed", "steps": "steps", "cfg_scale": "cfg_scale", 
            ...
            setting_parts = [f"{k.replace('_', ' ').capitalize()}: {remove_quotes(str(v))}" for k,v in data_json.items()]
            ...
            self._error = f"Invalid JSON in invokeai_metadata: {e}"; return False
        except Exception as e:
            self._error = f"Error parsing invokeai_metadata: {e}"; self._logger.error(f"InvokeAI metadata parsing error: {e}", exc_info=True); return False
    ...
    def _parse_sd_metadata_format(self) -> bool: # New name for original _invoke_sd_metadata
        try:
            ...
            if not isinstance(data_json, dict):
                self._error = "sd-metadata is not a valid JSON dictionary."; return False

            image_data = data_json.get("image", {})
            if not image_data:
                self._error = "'image' field missing in sd-metadata."; return False
            ...
            # Populate parameters
            param_map = {"model": "model", "sampler": "sampler_name", "seed": "seed", 
                         "cfg_scale": "cfg_scale", "steps": "steps", "width": "width", "height": "height"}
            ...
            if image_data.get("width") is not None: self._width = str(image_data.get("width"))
            if image_data.get("height") is not None: self._height = str(image_data.get("height"))
            if "size" in self._parameter and self._width != "0" and self._height != "0":
                 self._parameter["size"] = f"{self._width}x{self._height}"
            ...
            self._error = f"Invalid JSON in sd-metadata: {e}"; return False
        except Exception as e:
            self._error = f"Error parsing sd-metadata: {e}"; self._logger.error(f"InvokeAI sd-metadata parsing error: {e}", exc_info=True); return False

    def _parse_dream_format(self) -> bool: # New name for original _invoke_dream
        try:
            data_str = self._info.get("Dream", "")
            if not data_str: self._error = "'Dream' string is empty."; return False

            # Regex to separate prompt from settings: "Prompt" -s 100 -S 123 ...
            pattern = r'"(.*?)"\s*(-\S.*)?$' # Group 1: Prompt, Group 2: Settings string
            match = re.search(pattern, data_str)
            if not match: self._error = "Could not parse 'Dream' string structure."; return False
            ...
            # Map to canonical
            if "s" in parsed_settings and "steps" in self._parameter: self._parameter["steps"] = parsed_settings["s"]
            if "S" in parsed_settings and "seed" in self._parameter: self._parameter["seed"] = parsed_settings["S"]
            if "C" in parsed_settings and "cfg_scale" in self._parameter: self._parameter["cfg_scale"] = parsed_settings["C"]
            if "A" in parsed_settings and "sampler_name" in self._parameter: self._parameter["sampler_name"] = parsed_settings["A"]
            
            if "W" in parsed_settings: self._width = parsed_settings["W"]
            if "H" in parsed_settings: self._height = parsed_settings["H"]
            if "size" in self._parameter and self._width != "0" and self._height != "0":
                 self._parameter["size"] = f"{self._width}x{self._height}"
            ...
            for k, v in parsed_settings.items():
                ...
                for name, short_key in self.DREAM_MAPPING.items():
                    if short_key == k: display_key = name; break
            ...
            self._error = f"Error parsing Dream string: {e}"; self._logger.error(f"InvokeAI Dream parsing error: {e}", exc_info=True); return False
