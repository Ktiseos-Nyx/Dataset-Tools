...
from .base_model_parser import BaseModelParser, ModelParserStatus # Your base class
...
class GGUFParser(BaseModelParser):
    ...
    def _read_metadata_value(self, f, value_type_enum_val: int):
        ...
        if value_type == GGUFValueType.UINT8: return struct.unpack("<B", f.read(1))[0]
        elif value_type == GGUFValueType.INT8: return struct.unpack("<b", f.read(1))[0]
        elif value_type == GGUFValueType.UINT16: return struct.unpack("<H", f.read(2))[0]
        elif value_type == GGUFValueType.INT16: return struct.unpack("<h", f.read(2))[0]
        elif value_type == GGUFValueType.UINT32: return struct.unpack("<I", f.read(4))[0]
        elif value_type == GGUFValueType.INT32: return struct.unpack("<i", f.read(4))[0]
        elif value_type == GGUFValueType.FLOAT32: return struct.unpack("<f", f.read(4))[0]
        elif value_type == GGUFValueType.BOOL: return struct.unpack("?", f.read(1))[0]
        elif value_type == GGUFValueType.STRING: return self._read_string(f)
        elif value_type == GGUFValueType.UINT64: return struct.unpack("<Q", f.read(8))[0]
        elif value_type == GGUFValueType.INT64: return struct.unpack("<q", f.read(8))[0]
        elif value_type == GGUFValueType.FLOAT64: return struct.unpack("<d", f.read(8))[0]
            ...
            try:
                ...
                if array_len > 0 and array_len < 20: # Only try to read small arrays for demo
                    ...
                    nfo(f"[{self._logger_name}] GGUF metadata array of type {array_item_gtype.name} with len {array_len}. Content not fully parsed for display.")
        ...
        try:
            with open(self.file_path, "rb") as f:
                ...
                if self.gguf_version not in [1, 2, 3]: # Known GGUF versions
                     nfo(f"[{self._logger_name}] Encountered GGUF version {self.gguf_version}. Parser may have limitations.")
                ...
                nfo(f"[{self._logger_name}] GGUF v{self.gguf_version}, Tensors: {self.tensor_count}, Meta KVs: {self.metadata_kv_count}")
                ...
                for i in range(self.metadata_kv_count):
                    ...
                    try:
                        ...
                        nfo(f"[{self._logger_name}] Error reading GGUF metadata value for key '{key}': {ve_val}. Storing as error string.")
                        ...
                        nfo(f"[{self._logger_name}] Struct error reading GGUF metadata value for key '{key}': {se_val}. File might be truncated.")
