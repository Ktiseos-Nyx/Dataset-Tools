...
from ..correct_types import DownField, EmptyField, ExtensionType as Ext
...
class SchemaFileReader:
    ...
    def read_file(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return self._create_error_result(f"Error reading {suffix.upper()}: {str(e)}", suffix)
    
    def _read_json_file(self, file_path: str) -> Optional[Union[Dict, List]]:
    ...
    def _read_toml_file(self, file_path: str) -> Optional[Dict[str, Any]]:
    ...
    def _read_yaml_file(self, file_path: str) -> Optional[Union[Dict, List]]:
    ...
    def _read_xml_file(self, file_path: str) -> Optional[Dict[str, Any]]:
    ...
    def _xml_to_dict(self, element) -> Dict[str, Any]:
        ...
        return result if len(result) > 1 or '@attributes' in result else result.get(list(result.keys())[0]) if result else {}
    ...
    def _analyze_structure(self, data: Union[Dict, List], suffix: str) -> Dict[str, Any]:
    ...
    def _detect_common_patterns(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(key.lower() in config_indicators for key in data.keys()):
            patterns["appears_to_be_config"] = True
        
        # Check for API response patterns
        api_indicators = ['data', 'status', 'message', 'error', 'success']
        if any(key.lower() in api_indicators for key in data.keys()):
            patterns["appears_to_be_api_response"] = True
        
        # Check for metadata patterns
        metadata_indicators = ['metadata', 'meta', 'info', 'version', 'created', 'modified']
        if any(key.lower() in metadata_indicators for key in data.keys()):
        ...
        if any(key.lower() in workflow_indicators for key in data.keys()):
...
class StructuredDataAnalyzer:
    ...
    def analyze_comfyui_workflow(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            has_nodes = 'nodes' in data or any(key.isdigit() for key in data.keys())
    ...
    def extract_metadata_fields(self, data: Union[Dict, List]) -> Dict[str, Any]:
        """
...
def read_schema_file(file_path: str) -> Optional[Dict[str, Any]]:
    """
