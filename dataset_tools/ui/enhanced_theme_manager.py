...
class EnhancedThemeManager:
    ...
    def __init__(self, main_window: Qw.QMainWindow, settings: QSettings):
        ...
        self.current_palette_theme: Optional[str] = None
        self.current_qt_material_theme: Optional[str] = None
        
        # Load qt-themes palettes dynamically
        self._load_qt_themes_palettes()
        
        nfo("Enhanced ThemeManager initialized")
        nfo("Available systems: qt-material=%s, qt-themes=%s, unreal=%s", 
            QT_MATERIAL_AVAILABLE, QT_THEMES_AVAILABLE, UNREAL_STYLESHEET_AVAILABLE)
    ...
    def _apply_custom_qss_theme(self, theme_name: str, app: QApplication) -> bool:
        ...
        try:
            ...
            # Replace GitHub URLs
            stylesheet = re.sub(r'url\("(https://[^"]*github[^"]*\.png)"\)', replace_github_url, stylesheet)
            stylesheet = re.sub(r"url\('(https://[^']*github[^']*\.png)'\)", replace_github_url, stylesheet)
