...
class IconManager:
    ...
    def __init__(self, icons_directory: Optional[str] = None):
    ...
    def _colorize_svg(self, svg_path: str, color_type: str, size: QSize) -> QPixmap:
        ...
        """
        # Get the color first
        color = self.current_theme_colors.get(color_type, self.current_theme_colors.get("primary", QColor(128, 128, 128)))
    ...
    def _create_fallback_icon(self, icon_name: str, size: QSize) -> QIcon:
        ...
        if size.width() >= 16:
            ...
            painter.drawText(pixmap.rect(), QtCore.Qt.AlignmentFlag.AlignCenter, icon_name[:2].upper())
        
        painter.end()
        return QIcon(pixmap)
    
    def add_icon_to_button(self, button: Qw.QPushButton, icon_name: str, 
                          color_type: str = "primary", icon_size: QSize = QSize(16, 16)) -> None:
