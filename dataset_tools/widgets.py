...
class FileLoader(QtCore.QThread):
    ...
    def run(self):
        ...
        nfo(f"[FileLoader] Scan finished. Emitting results for folder: {self.folder_path}. File to select: {self.file_to_select_on_finish}")
        self.finished.emit(self.images, self.text_files, self.model_files, self.folder_path, self.file_to_select_on_finish)
    ...
    def populate_index_from_list(self, folder_item_paths: list[str] | None) -> tuple[list[str], list[str], list[str]]:
        ...
        print(f"--- DEBUG WIDGETS: Inspecting Ext (ExtensionType) from correct_types.py ---")
        ...
        for attr_name in expected_attrs:
            ...
            if has_attr:
                ...
                if len(print_val) > 100 : print_val = print_val[:100] + "..."
                print(f"DEBUG WIDGETS: Value of Ext.{attr_name}: {print_val}")
        print(f"--- END DEBUG WIDGETS ---")
        # --- End Debug prints ---
        ...
        if hasattr(Ext, 'IGNORE') and isinstance(Ext.IGNORE, list):
            ...
            nfo("[FileLoader] WARNING: Ext.IGNORE attribute not found or not a list in ExtensionType. Using empty ignore list.")
        ...
        for f_path_str in folder_item_paths:
            ...
            if total_items > 0:
                ...
                if progress_percent > current_progress_percent and (progress_percent % 5 == 0 or progress_percent == 100):
        ...
        nfo(f"[FileLoader] Categorized files: {len(local_images)} images, {len(local_text_files)} text/schema, {len(local_model_files)} models.")
