...
class TemplateProcessor:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self._original_input_data: Optional[str] = None
        self._input_json_object: Optional[Union[Dict, List]] = None
    ...
    def process_template(
        ...
        original_input_data: Optional[str] = None,
        input_json_object: Optional[Union[Dict, List]] = None,
    ) -> TemplateData:
    ...
    def _process_recursive(
        self, template: TemplateData, extracted_fields: ExtractedFields, context_data: ContextData
    ) -> TemplateData:
        """Recursively process template structures."""
        if isinstance(template, dict):
            return {key: self._process_recursive(value, extracted_fields, context_data) for key, value in template.items()}
    ...
    def _substitute_variables(
        self, template_string: str, extracted_fields: ExtractedFields, context_data: ContextData
    ) -> str:
    ...
    def _handle_input_string_original(self) -> str: return str(self._original_input_data) if self._original_input_data is not None else ""
    def _handle_input_json_as_string(self) -> str: return json.dumps(self._input_json_object, indent=2) if self._input_json_object is not None else ""
    def _handle_current_timestamp(self) -> str: import datetime; return datetime.datetime.now().isoformat()
    def _handle_file_path(self) -> str: return str(self._context_data.get("file_path_original", ""))
    def _handle_file_name(self) -> str:
        file_path = self._context_data.get("file_path_original", ""); return __import__("pathlib").Path(file_path).name if file_path else ""
    def _handle_file_extension(self) -> str: return str(self._context_data.get("file_extension", ""))
...
class TemplateValidator:
    ...
    def __init__(self, logger: Optional[logging.Logger] = None):
        ...
        self.variable_pattern = re.compile(r'\$([\w.]+)')
    def validate_template(self, template: TemplateData, available_fields: List[str], available_context_keys: List[str]) -> bool:
    ...
    def _validate_recursive(self, template: TemplateData, fields: List[str], context_keys: List[str]):
        if isinstance(template, dict): [self._validate_recursive(v, fields, context_keys) for v in template.values()]
        elif isinstance(template, list): [self._validate_recursive(i, fields, context_keys) for i in template]
        elif isinstance(template, str): self._validate_string_template(template, fields, context_keys)
    def _validate_string_template(self, t_string: str, fields: List[str], context_keys: List[str]):
        special_vars = ["INPUT_STRING_ORIGINAL_CHUNK", "INPUT_JSON_OBJECT_AS_STRING", "CURRENT_TIMESTAMP", "FILE_PATH", "FILE_NAME", "FILE_EXTENSION"]
        for match in self.variable_pattern.finditer(t_string):
            var = match.group(1)
            if var in special_vars: continue
            if var.startswith("CONTEXT."):
                if var.split('.')[1] not in context_keys: self.warnings.append(f"Context var '${var}' may not be available.")
            elif var.split('.')[0] not in fields: self.warnings.append(f"Field var '${var}' may not be available.")
...
class StandardTemplates:
    ...
    def a1111_template() -> Dict[str, Any]:
        return {"tool": "AUTOMATIC1111", "prompt": "$prompt", "negative_prompt": "$negative_prompt", "parameters": {"steps": "$parameters.steps", "sampler": "$parameters.sampler", "cfg_scale": "$parameters.cfg_scale", "seed": "$parameters.seed", "width": "$CONTEXT.width", "height": "$CONTEXT.height", "model": "$parameters.model", "model_hash": "$parameters.model_hash", "version": "$parameters.version"}}


class TemplateBuilder:
    """Builder class for creating template structures programmatically."""
    def __init__(self): self.template_data: Dict[str, Any] = {}
    def add_field(self, key: str, value: Any) -> "TemplateBuilder": self.template_data[key] = value; return self
    def add_variable(self, key: str, var_name: str) -> "TemplateBuilder": self.template_data[key] = f"${var_name}"; return self
    @classmethod
    def create_standard_ai_template(cls, tool_name: str, **kwargs) -> "TemplateBuilder":
        builder = cls().add_field("tool", tool_name)
        if kwargs.get("include_prompts", True): builder.add_variable("prompt", "prompt").add_variable("negative_prompt", "negative_prompt")
        if kwargs.get("extra_fields"): [builder.add_variable(k, v) for k, v in kwargs["extra_fields"].items()]
        return builder
    def build(self) -> Dict[str, Any]: return self.template_data
...
class OutputFormatter:
    ...
    def format_output(self, processed_template: TemplateData, context_data: Optional[ContextData] = None, **kwargs) -> Dict[str, Any]:
        if not isinstance(processed_template, dict):
            self.logger.warning("Processed template is not a dictionary; wrapping it."); processed_template = {"data": processed_template}
        output = processed_template.copy()
        if kwargs.get("cleanup_empty", True): output = self._cleanup_empty_values(output)
        format_type = kwargs.get("format_type", "standard")
        if format_type == "standard": output = self._apply_standard_formatting(output, context_data or {})
        elif format_type == "minimal": output = self._apply_minimal_formatting(output)
        if kwargs.get("add_metadata", True): output = self._add_processing_metadata(output)
        return output
    def _apply_standard_formatting(self, output: Dict[str, Any], context: ContextData) -> Dict[str, Any]:
        formatted = output.copy()
        if "tool" in formatted and isinstance(formatted.get("tool"), str): formatted["tool"] = formatted["tool"].strip()
        if "parameters" in formatted and isinstance(formatted.get("parameters"), dict):
            if "width" not in formatted["parameters"] and context.get("width", 0) > 0: formatted["parameters"]["width"] = context["width"]
            if "height" not in formatted["parameters"] and context.get("height", 0) > 0: formatted["parameters"]["height"] = context["height"]
        return formatted
    def _apply_minimal_formatting(self, output: Dict[str, Any]) -> Dict[str, Any]:
        return {f: output[f] for f in ["tool", "prompt", "parameters"] if f in output and not self._is_empty_value(output[f])}
    def _add_processing_metadata(self, output: Dict[str, Any]) -> Dict[str, Any]:
        if "_metadata" not in output: output["_metadata"] = {}
        output["_metadata"].update({"processed_at": __import__("datetime").datetime.now().isoformat(), "processor": "MetadataEngine", "template_processed": True})
        return output
    def _cleanup_empty_values(self, data: TemplateData) -> TemplateData:
        if isinstance(data, dict): return {k: v for k, v in ((k, self._cleanup_empty_values(v)) for k, v in data.items()) if not self._is_empty_value(v)}
        if isinstance(data, list): return [v for item in data if not self._is_empty_value(v := self._cleanup_empty_values(item))]
        return data
    def _is_empty_value(self, value: Any) -> bool:
        if value is None: return True
        if isinstance(value, str) and not value.strip(): return True
        if isinstance(value, (dict, list)) and not value: return True
        return False
...
def process_template(template: TemplateData, extracted_fields: ExtractedFields, context_data: ContextData, **kwargs) -> TemplateData:
...
def test_template_system():
    ...
    extracted = {"prompt": "beautiful landscape", "negative_prompt": "blurry", "parameters": {"steps": 20, "sampler": "Euler a", "seed": 42}, "tool_name": "TestTool"}
    context = {"width": 512, "height": 768, "file_path_original": "test.png", "file_extension": "png"}
