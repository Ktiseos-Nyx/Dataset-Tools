...
class RuleOperators:
    ...
    def exists_and_is_dictionary(self, data: Any, rule: RuleDict, context: ContextData) -> bool:
        ...
        self.logger.debug(
            f"exists_and_is_dictionary: data type {type(data)}, "
            f"is_dict={is_dict}, is_not_empty={is_not_empty}"
        )
    ...
    def _get_json_object(self, data: Any) -> Optional[Union[Dict, List]]:
    ...
    def _handle_json_path_query(self, data: Any, rule: RuleDict) -> bool:
        ...
        try:
            ...
            if json_query_type == "has_numeric_string_keys":
                return (isinstance(json_obj, dict) and 
                       any(key.isdigit() for key in json_obj))
                ...
                return any(
                    ...
                    node_val.get("type") in class_types
    ...
    def _handle_a1111_parameter_string(self, context: ContextData) -> tuple[Any, bool]:
        ...
        try:
            ...
            if (isinstance(wrapper, dict) and 
                "parameters" in wrapper and 
                isinstance(wrapper["parameters"], str)):
...
class RuleEngine:
    ...
    def _apply_operator(self, operator: str, data: Any, rule: RuleDict, context: ContextData) -> RuleResult:
        ...
        try:
            ...
            self.logger.error(f"Error applying operator '{operator}' for rule '{rule_comment}': {e}", exc_info=True)
            return False
...
def create_rule_engine(config_path: Optional[Path] = None) -> RuleEngine:
    """
...
class RuleBuilder:
    ...
    @classmethod
    def json_path_rule(cls, source_type: str, json_path: str, source_key: str = None, comment: str = None) -> RuleDict:
...
class RuleValidator:
    ...
    def _validate_simple_rule(self, rule: RuleDict) -> bool:
        ...
        if operator in ["equals", "equals_case_insensitive", "contains", "contains_case_insensitive",
                       "startswith", "endswith", "is_in_list"]:
