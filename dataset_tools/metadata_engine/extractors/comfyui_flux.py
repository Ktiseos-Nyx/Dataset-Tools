...
class ComfyUIFluxExtractor:
            ...
            if "FluxScheduler" in class_type or "FlowMatchEulerDiscreteScheduler" in class_type:
                ...
                if widgets:
                    scheduler_params.update({
                        ...
                        "denoise": widgets[1] if len(widgets) > 1 and isinstance(widgets[1], (int, float)) else 1.0,
                    })
            ...
            # FLUX samplers
                ...
                if widgets:
                    scheduler_params.update({
                        ...
                        "steps": widgets[0] if len(widgets) > 0 and isinstance(widgets[0], (int, float)) else 20,
                        "max_shift": widgets[1] if len(widgets) > 1 and isinstance(widgets[1], (int, float)) else 1.15,
                        "base_shift": widgets[2] if len(widgets) > 2 and isinstance(widgets[2], (int, float)) else 0.5,
                    })
    ...
    def _detect_flux_workflow(
        ...
        flux_indicators = [
            "FluxCheckpointLoader", "FluxUNETLoader", "FluxVAELoader",
            "FluxGuidance", "FluxScheduler", "FluxSampler",
            "FlowMatchEulerDiscreteScheduler", "T5TextEncode",
            "DiffusionModel", "FluxModelLoader"
        ]
